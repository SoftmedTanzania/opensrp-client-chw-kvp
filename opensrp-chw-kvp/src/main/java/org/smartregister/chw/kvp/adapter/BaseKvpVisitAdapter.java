...
public class BaseKvpVisitAdapter extends RecyclerView.Adapter<BaseKvpVisitAdapter.MyViewHolder> {
    private Map<String, BaseKvpVisitAction> kvpVisitActionList;
    private Context context;
    private BaseKvpVisitContract.View visitContractView;

    public BaseKvpVisitAdapter(Context context, BaseKvpVisitContract.View view, LinkedHashMap<String, BaseKvpVisitAction> myDataset) {
        kvpVisitActionList = myDataset;
    ...
    @NotNull
    @Override
    public BaseKvpVisitAdapter.MyViewHolder onCreateViewHolder(@NotNull ViewGroup parent,
                                                               int viewType) {
        View v = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.kvp_visit_item, parent, false);
     ...
     * @return
     */
    private BaseKvpVisitAction getByPosition(int position) {
        int count = -1;
        for (Map.Entry<String, BaseKvpVisitAction> entry : kvpVisitActionList.entrySet()) {
    ...
    public void onBindViewHolder(@NotNull MyViewHolder holder, int position) {
        ...
        BaseKvpVisitAction kvpVisitAction = getByPosition(position);
        if (kvpVisitAction == null)

            return;

        if (!kvpVisitAction.isEnabled()) {
        ...
        String title = MessageFormat.format("{0}<i>{1}</i>",
                kvpVisitAction.getTitle(),
                kvpVisitAction.isOptional() ? " - " + context.getString(R.string.optional) : ""
        );
        holder.titleText.setText(Html.fromHtml(title));
        if (StringUtils.isNotBlank(kvpVisitAction.getSubTitle())) {

            if (kvpVisitAction.isEnabled()) {
                holder.descriptionText.setVisibility(View.VISIBLE);
                holder.invalidText.setVisibility(View.GONE);
                holder.descriptionText.setText(kvpVisitAction.getSubTitle());

                boolean isOverdue = kvpVisitAction.getScheduleStatus() == BaseKvpVisitAction.ScheduleStatus.OVERDUE &&
                        kvpVisitAction.isEnabled();
                ...
                holder.invalidText.setText(Html.fromHtml("<i>" + kvpVisitAction.getDisabledMessage() + "</i>"));
            }
        } else {
            holder.descriptionText.setVisibility(View.GONE);
        }

        int color_res = getCircleColor(kvpVisitAction);
        ...
        bindClickListener(holder.getView(), kvpVisitAction);
    }

    private int getCircleColor(BaseKvpVisitAction kvpVisitAction) {

        int color_res;
        boolean valid = kvpVisitAction.isValid() && kvpVisitAction.isEnabled();
        if (!valid)
            return R.color.transparent_gray;

        switch (kvpVisitAction.getActionStatus()) {
            case PENDING:
    ...
    private void bindClickListener(View view, final BaseKvpVisitAction kvpVisitAction) {
        if (!kvpVisitAction.isEnabled() || !kvpVisitAction.isValid()) {
            view.setOnClickListener(null);
            return;
        }

        view.setOnClickListener(v -> {
            if (StringUtils.isNotBlank(kvpVisitAction.getFormName())) {
                visitContractView.startForm(kvpVisitAction);
            } else {
                visitContractView.startFragment(kvpVisitAction);
            }
