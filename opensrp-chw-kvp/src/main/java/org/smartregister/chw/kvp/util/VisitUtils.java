...
public class VisitUtils {
    ...
    public static List<Visit> getVisitsOnly(String memberID, String visitName) {
        return new ArrayList<>(KvpLibrary.getInstance().visitRepository().getVisits(memberID, visitName));
    }

    public static List<VisitDetail> getVisitDetailsOnly(String visitID) {
        return KvpLibrary.getInstance().visitDetailsRepository().getVisits(visitID);
    }
    ...
    public static void processVisits(String baseEntityID) throws Exception {
        processVisits(KvpLibrary.getInstance().visitRepository(), KvpLibrary.getInstance().visitDetailsRepository(), baseEntityID);
    }
    ...
    public static void processVisits(List<Visit> visits, VisitRepository visitRepository, VisitDetailsRepository visitDetailsRepository) throws Exception {
        ...
        for (Visit v : visits) {
            if (!v.getProcessed()) {
                ...
                baseEvent.addDetails(Constants.KVP_VISIT_GROUP, visitGroupId);
    ...
    public static Date getDateFromString(String dateStr) {
        try {
            return NCUtil.getSaveDateFormat().parse(dateStr);
